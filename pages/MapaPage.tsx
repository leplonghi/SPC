    ...
    <div className="h-[calc(100vh-5rem)] flex flex-col md:flex-row overflow-hidden relative">
      ...
      <div className="w-full md:w-96 bg-white border-r border-slate-200 flex flex-col z-20 shadow-xl overflow-hidden">
        ...
        <div className="p-6 border-b border-slate-100 bg-slate-50/50">
          <h2 className="text-xl font-extrabold text-brand-dark mb-4 flex items-center gap-2">
          ...
          <div className="relative group">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue transition-all"
              value={searchTerm}
          ...
          <div className="flex items-center justify-between mt-4">
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${showFilters ? 'bg-brand-blue text-white shadow-lg shadow-brand-blue/20' : 'bg-white text-slate-600 border border-slate-200'}`}
            >
        ...
        {showFilters && (
          <div className="p-6 border-b border-slate-200 bg-brand-blue/5 space-y-4 animate-in slide-in-from-top-4 duration-300">
            <div>
            ...
            <div className="divide-y divide-slate-100">
              {filteredAssets.map(asset => (
                <button
                  ...
                  className={`w-full p-5 text-left transition-all hover:bg-white hover:shadow-md border-l-4 ${selectedAsset?.id === asset.id ? 'bg-white border-brand-blue shadow-inner' : 'border-transparent'}`}
                >
                  <div className="flex gap-4">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex gap-1">
                        <span className="px-2 py-0.5 bg-brand-blue/10 text-brand-blue text-[9px] rounded-full font-bold uppercase">{asset.protection_level}</span>
          ...
          <div className="flex-grow overflow-y-auto">
            ...
            <div className="p-8 space-y-10">
              <header className="space-y-4">
                <div className="flex gap-2">
                  <span className="px-3 py-1 bg-brand-red text-white text-[10px] font-extrabold rounded-full uppercase tracking-widest">{selectedAsset.protection_level}</span>
                  <span className="px-3 py-1 bg-brand-blue/10 text-brand-blue text-[10px] font-extrabold rounded-full uppercase tracking-widest">{selectedAsset.status}</span>
                </div>
                <h1 className="text-3xl font-black text-brand-dark leading-tight">{selectedAsset.name}</h1>
                <p className="flex items-center gap-2 text-slate-500 font-medium">
                  <MapPin size={18} className="text-brand-red" /> {selectedAsset.address_text}, {selectedAsset.municipality}
                </p>
              </header>

              <section className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
                <h3 className="text-sm font-black text-brand-dark uppercase tracking-widest mb-4 flex items-center gap-2">
                  <FileText size={18} className="text-brand-blue" /> Base Legal
                </h3>
              ...
              <section className="space-y-4">
                <h3 className="text-sm font-black text-brand-dark uppercase tracking-widest flex items-center gap-2">
                  <Info size={18} className="text-brand-blue" /> Descrição Histórica
                </h3>
