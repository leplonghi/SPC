  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <section className="bg-[#2D2D2D] pt-32 pb-20 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[#CC343A] opacity-5 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/2"></div>
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-[#5283A9] opacity-5 rounded-full blur-[100px] translate-y-1/2 -translate-x-1/2"></div>
        
        <div className="relative max-w-7xl mx-auto px-4 text-center space-y-8">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#5283A9]/20 text-[#5283A9] border border-[#5283A9]/30 rounded-full text-xs font-black uppercase tracking-[0.2em]">
          ...
          <div className="max-w-2xl mx-auto relative group mt-10">
            ...
            <input 
              ...
              className="w-full pl-16 pr-6 py-6 bg-white/5 border-2 border-white/10 rounded-[2rem] text-lg text-white font-medium focus:outline-none focus:ring-8 focus:ring-[#CC343A]/5 focus:border-[#CC343A] transition-all"
              value={searchTerm}
        ...
        <aside className="lg:w-64 flex-shrink-0 space-y-10">
          <div className="space-y-4">
            <h3 className="text-xs font-black text-[#2D2D2D] uppercase tracking-widest flex items-center gap-2">
              <Filter size={14} className="text-[#CC343A]" /> Categorias
            </h3>
              ...
              {categories.map(cat => (
                <button
                  ...
                  className={`flex items-center justify-between px-4 py-3 rounded-xl text-sm font-bold transition-all ${
                    activeCategory === cat 
                    ? 'bg-[#5283A9] text-white shadow-lg' 
          ...
          <div className="p-6 bg-[#CC343A]/5 rounded-[2rem] border border-[#CC343A]/10 space-y-4">
            <h4 className="text-xs font-black text-[#CC343A] uppercase tracking-widest">Dúvida Técnica?</h4>
            ...
            <button className="w-full py-3 bg-[#CC343A] text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#b02c32] transition-colors">
              Falar com Jurídico
            </button>
          </div>
        </aside>
        ...
        <section className="flex-grow space-y-6">
          <div className="flex items-center justify-between mb-8 px-4">
            ...
            <div className="flex gap-2">
               <button className="p-2 bg-white rounded-lg border border-slate-200 text-slate-400 hover:text-[#5283A9] transition-colors"><Archive size={20} /></button>
            </div>
          </div>
          ...
          {filteredDocs.length === 0 ? (
            <div className="text-center py-32 bg-white rounded-[3rem] border border-slate-100">
              ...
              <button onClick={() => {setSearchTerm(''); setActiveCategory('Todos')}} className="mt-4 text-[#5283A9] font-black uppercase tracking-widest text-xs">Limpar filtros</button>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-4">
              {filteredDocs.map(doc => (
                <div key={doc.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 hover:border-[#5283A9]/30 hover:shadow-xl transition-all group flex flex-col md:flex-row md:items-center gap-8">
                  <div className={`w-16 h-16 rounded-2xl flex items-center justify-center flex-shrink-0 ${
                    doc.category === 'Lei' ? 'bg-[#CC343A]/10 text-[#CC343A]' : 
                    doc.category === 'Decreto' ? 'bg-[#5283A9]/10 text-[#5283A9]' : 
                  ...
                  <div className="flex-grow space-y-2">
                    <div className="flex flex-wrap items-center gap-3">
                      <span className={`text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full ${
                        doc.category === 'Lei' ? 'bg-[#CC343A] text-white' : 
                        doc.category === 'Decreto' ? 'bg-[#5283A9] text-white' : 
                    ...
                    <h3 className="text-xl font-black text-[#2D2D2D] group-hover:text-[#5283A9] transition-colors">{doc.title}</h3>
                  ...
                  <div className="flex flex-col gap-3 min-w-[140px]">
                    <button className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-[#5283A9] text-white rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-[#436d8d] transition-all active:scale-95">
      ...
      <section className="bg-white py-24 border-t border-slate-100">
        <div className="max-w-7xl mx-auto px-4">
           <div className="flex flex-col md:flex-row justify-between items-end gap-8 mb-16">
              <div className="space-y-4">
                 <h2 className="text-3xl font-black text-[#2D2D2D] tracking-tighter uppercase">Destaques e Formulários</h2>
                 <div className="h-1.5 w-16 bg-[#CC343A] rounded-full"></div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                ...
                { title: 'Checklist para Reformas', color: 'bg-[#5283A9]', icon: FileText },
                { title: 'Guia de Tombamento Federal', color: 'bg-slate-900', icon: BookOpen }
              ].map((item, i) => (
                <div key={i} className="group relative overflow-hidden bg-slate-50 rounded-[2.5rem] p-10 border border-slate-100 hover:shadow-2xl transition-all">
                   ...
                   <h4 className="text-xl font-black text-[#2D2D2D] mb-4">{item.title}</h4>
                   <button className="flex items-center gap-2 text-[10px] font-black text-[#5283A9] uppercase tracking-widest group-hover:gap-4 transition-all">
